...
public class LoginDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		addBooleanOption(composite, GeneralPreferencePage.AUTO_LOGIN, "Login &automatically at startup");
		addBooleanOption(composite, GeneralPreferencePage.NON_RECT_WINDOW, "Run in a &non-rectangular window");
	...
	private void addBooleanOption(Composite composite, final String preferenceId, String text) {
		final Button booleanOption = new Button(composite, SWT.CHECK);
		booleanOption.setText(text);
		booleanOption.setLayoutData(new GridData(SWT.BEGINNING, SWT.CENTER, true,
				true, 2, 1));
		booleanOption.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				IEclipsePreferences prefs = new ConfigurationScope()
						.getNode(Application.PLUGIN_ID);
				prefs.putBoolean(preferenceId, booleanOption
						.getSelection());
			}
		});
		IPreferencesService service = Platform.getPreferencesService();
		boolean auto_login = service.getBoolean(Application.PLUGIN_ID,
				preferenceId, false, null);

		booleanOption.setSelection(auto_login);
	}
